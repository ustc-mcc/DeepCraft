...
api_key = "PYlPLaVqw1T6wnZCm5pl24Z2"
api_secert = "f82614964bc5f772bd5623fe9b559137"
bdr = BaiduRest("test_python", api_key, api_secert)

gen_audio_file = 'VideoSR/static/audios/result.wav'
pro_audio_file = 'VideoSR/static/audios/pron.wav'

def run_text2audio(sentence):
	bdr.text2audio(sentence, gen_audio_file)

def run_audio2text(pro_audio_file):
	result = bdr.audio2text(pro_audio_file)

def run_rate(rawtxt, gentxt):
	score = float(len(set(rawtxt) & set(gentxt))) / len(set(rawtxt) | set(gentxt))
	return score
...
#    #生成
#    start = time.time()
#    bdr.text2audio("My name is Mike", "out.wav")
...
#    #识别
#    start = time.time()
#    result = bdr.audio2text("test_en.wav")
